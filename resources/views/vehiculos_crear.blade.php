...
<div class="w3-modal-content">
    ...
    @else
        ...
        <div class="w3-container">
            ...
            <form action="{{ route('vehiculo.store',['cliente_id' => $cliente_id]) }}" method="POST">
                @csrf
                <p>
                    <input name="placa" class="w3-input w3-border" value="{{ old('placa') }}" type="text" maxlength="50" placeholder="Placa - Serial">
                </p>
                {!! $errors->first('placa', '<small class="w3-text-red">:message</small>') !!}

                <p>
                    <select class="w3-select w3-border" name="tipo">
                        <option @if(old('tipo')=='') selected @endif disable value="">Tipo</option>
